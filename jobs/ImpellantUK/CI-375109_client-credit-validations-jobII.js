      ...
      .then((resp) => {
        ...
        if (resp.data.count > 0) {
          let thisFilter = resp.data.data.filter((item) => {
            console.log("itemmmm, ", item, item.text3.filter((el) => employmentType.includes(el)));
            if (
              !(item.date1 < startDate ||
                item.text3.filter((el) => employmentType.includes(el)).length <=
                0 ||
                item.text2 !== "Approved")
            ) {
              return item;
